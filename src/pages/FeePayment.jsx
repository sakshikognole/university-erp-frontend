...
// Node/Express backend â-- auth, create-order, verify
...
// Spring Boot backend â-- fee payment combinations
 ...
 *   id, paymentId, paymentTitles (string[] â-- title names), totalAmount, createdAt
 *
 * allTitles: PaymentTitle[] fetched from /api/payment-titles (all pages)
 *
 * Mapped to:
 *   _id              â-- MongoDB id of the combination (used as feeRecordId in Razorpay)
 *   feeType          â-- joined title names (e.g. "Bus Fee + Hostel Fee")
 *   totalFeeAmount   â-- gross sum of title.amount (before discount)
 *   discountAmount   â-- total discount = grossAmount - totalAmount
 *   paidAmount       â-- starts at 0; updated locally after successful payment
 *   remainingAmount  â-- net payable = totalAmount (already discount-applied)
 *   status           â-- PENDING / PAID / PARTIAL
 */
const mapCombinationToFeeRecord = (combo, allTitles = []) => {
  ...
  return {
    ...
    feeType,
    totalFeeAmount:  resolvedGross,   // gross â-- shown in "Total Fee Amount" column
    discountAmount,                   // real discount â-- shown in "Discount" column
    paidAmount:      0,
    remainingAmount: combo.totalAmount, // net payable â-- what Razorpay charges
    status:          'PENDING',
  };
};
...
const FeePayment = () => {
  ...
  const handlePay = async (record) => {
    ...
    try {
      ...
      const options = {
        ...
        name: 'University ERP',
        description: `Fee Payment - â-¹${payAmountInRupees.toLocaleString('en-IN')}`,
  ...
  return (
    <div className="page-container">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ fontWeight: 600, fontSize: '0.95rem' }}>
                          â-¹{(record.totalFeeAmount || 0).toLocaleString('en-IN')}
                        </td>
                        <td style={{ color: '#047857', fontWeight: 600, fontSize: '0.95rem' }}>
                          {(record.discountAmount || 0) > 0
                            ? `- â-¹${(record.discountAmount || 0).toLocaleString('en-IN')}`
                            : 'â-¹0'}
                        </td>
                        <td style={{ fontWeight: 600, color: '#2563eb', fontSize: '0.95rem' }}>
                          â-¹{(record.paidAmount || 0).toLocaleString('en-IN')}
                        </td>
                        <td
                          style={{
                            fontWeight: 700,
                        ...
                        >
                          â-¹{(record.remainingAmount || 0).toLocaleString('en-IN')}
                        </td>
                        <td>
                          <span
                            style={{
      ...
      {/* â--â-- Selected Record Detail Card â--â-- */}
      {selectedRecord && (
        <div
          ref={detailRef}
          className="card"
            ...
            >
              ...
              {selectedRecord.degree && (
                <div style={detailItemStyle}>
                  ...
                  <div style={detailValueStyle}>
                    {selectedRecord.degree}
                    {selectedRecord.class ? ` â-- ${selectedRecord.class}` : ''}
              ...
              {selectedRecord.academicYear && (
                <div style={detailItemStyle}>
                  ...
                  <div style={detailValueStyle}>
                    {selectedRecord.academicYear}
                    {selectedRecord.semester ? ` â-¢ ${selectedRecord.semester}` : ''}
                  </div>
                </div>
              )}
              ...
              <div style={detailItemStyle}>
                ...
                <div style={{ ...detailValueStyle, fontWeight: 700 }}>
                  â-¹{(selectedRecord.totalFeeAmount || 0).toLocaleString('en-IN')}
                </div>
              </div>
              <div style={detailItemStyle}>
                <div style={detailLabelStyle}>
                ...
                <div style={{ ...detailValueStyle, color: '#047857' }}>
                  {(selectedRecord.discountAmount || 0) > 0
                    ? `- â-¹${(selectedRecord.discountAmount || 0).toLocaleString('en-IN')}`
                    : 'â-¹0'}
                </div>
              </div>
              <div style={detailItemStyle}>
              ...
              <div style={detailItemStyle}>
                ...
                <div style={{ ...detailValueStyle, color: '#2563eb', fontWeight: 700 }}>
                  â-¹{(selectedRecord.paidAmount || 0).toLocaleString('en-IN')}
                </div>
              </div>
              <div style={detailItemStyle}>
                <div style={detailLabelStyle}>
                ...
                >
                  â-¹{(selectedRecord.remainingAmount || 0).toLocaleString('en-IN')}
                </div>
              </div>
                  ...
                  {selectedRecord.transactions.map((txn, idx) => (
                    ...
                    >
                      ...
                      <span style={{ fontWeight: 700, color: '#111827' }}>
                        â-¹{(txn.amount || 0).toLocaleString('en-IN')}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                >
                  {processingId === selectedRecord._id ? (
                    ...
                    <>
                      <CreditCard size={17} />
                      <span>
                        Pay â-¹{(selectedRecord.remainingAmount || 0).toLocaleString('en-IN')}
                      </span>
                    </>
                  )}
                </button>
