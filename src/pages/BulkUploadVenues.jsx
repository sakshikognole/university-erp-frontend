...
// Must match backend constants â-- same rules as Add/Edit Venue
  ...
  const parseCSV = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!VENUE_ID_REGEX.test(rawVenueId)) {
        parseErrors.push(`Row ${i + 1}: Invalid Venue ID "${rawVenueId}" â-- must contain only letters and digits, optionally separated by hyphens (e.g. HALL-101, LAB-CS-01)`);
        continue;
      }

      const rawName = row.name.trim();
      if (!VENUE_NAME_REGEX.test(rawName)) {
        parseErrors.push(`Row ${i + 1}: Invalid Venue Name "${rawName}" â-- must start with a letter and may only contain letters, digits, spaces, or basic punctuation`);
        continue;
      }
