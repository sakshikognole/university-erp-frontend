                  ...
                  <tbody>
                    {paginatedData.map((vehicle, index) => (
                      <tr key={vehicle._id || vehicle.vehicleId}>
                        ...
                        <td>{vehicle.vehicleNumber || 'â--'}</td>
                        <td>{vehicle.capacity ? `${vehicle.capacity} seats` : 'â--'}</td>
                        <td>
                          <span
                            style={{
                              padding: '2px 8px',
                  ...
                  <tbody>
                    {paginatedData.map((pass, index) => (
                      <tr key={pass._id || pass.passId}>
                        ...
                        <td>
                          {pass.validTill ? new Date(pass.validTill).toLocaleDateString() : 'â--'}
                        </td>
                        <td style={{ textAlign: 'right' }}>
                          <div className="action-buttons-cell" style={{ justifyContent: 'flex-end' }}>
                            <button
            ...
            <div className="delete-item-preview">
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                  {deleteConfirm.type === 'route'
                    ? `Driver: ${deleteConfirm.item.driverName} â-¢ Vehicle: ${deleteConfirm.item.vehicleName}`
                    : deleteConfirm.type === 'driver'
                    ? `Driver ID: ${deleteConfirm.item.driverId} ${deleteConfirm.item.phone ? `â-¢ ${deleteConfirm.item.phone}` : ''}`
                    : deleteConfirm.type === 'vehicle'
                    ? `Vehicle ID: ${deleteConfirm.item.vehicleId} ${deleteConfirm.item.vehicleNumber ? `â-¢ ${deleteConfirm.item.vehicleNumber}` : ''}`
                    : `Student: ${deleteConfirm.item.studentId} â-¢ Route #${deleteConfirm.item.routeId}`}
                </div>
              </div>
